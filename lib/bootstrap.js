	...
	task.updateCategoryArticles = function (next) {
		let filesToArticles = function (categories) {
			categories.forEach((category) => {
				...
				category.articles = category.articleFiles.map((article) => {
					let uriStr = category.uri
					if (uriStr.slice(-1) !== '/') uriStr += '/'
					uriStr += article.replace(/.md/, '')
					return helpers.findArticle(uriStr, context.articles)
				})
