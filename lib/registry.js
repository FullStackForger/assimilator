				...
				forger.doWhile((next) => {
					...
					myhead.readFrom(fullPath).then((data) => {
						...
						if (data.title) {
							...
							if (uri.search(/-series\/.[\w\d-]*$/) > -1
								&& settings.blog.series
								&& settings.blog.series.longTitle) {

								let titleCategory = uriParts[uriParts.length - 2]
								let delimiter = settings.blog.series && settings.blog.series.longTitleDelimiter
									? settings.blog.series.longTitleDelimiter : ' '
								data.title = titleCategory + delimiter + data.title;
							}
